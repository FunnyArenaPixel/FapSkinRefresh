...
> **⚠️ 方案演进说明**（原文已标记删除线，保留以记录思考过程）

> ~~定期（如每 10 秒）检测附近玩家的渲染状态，发现隐身玩家时立即通过 PyRpc 请求 `RequestSkinRefreshEvent`，配合服务端的精准刷新（半径过滤），实现即时修复 + 极低开销。~~

> **为什么不可行：** 网易中国版 ModSDK 的客户端 Python API（`ClientSystem` / `CompFactory`）能获取的是实体的**逻辑数据**——实体 ID、位置坐标、类型标识符、血量等（参见[中国版 ModSDK 文档 — 实体组件](https://mc.163.com/dev/article/default/highquality/191213.html)，如 `GetEntities`、`GetEntityById` 等接口）。但 **ModSDK 没有任何接口能查询实体的渲染层面状态**——即"该实体的皮肤贴图是否已加载""模型是否正确渲染""玩家是否在画面上可见"这些信息属于客户端渲染引擎的内部状态，ModSDK 完全不暴露这一层。因此客户端**无法主动检测到某个玩家已经隐身**，"发现隐身 → 请求修复"的反应式链路从根本上不成立。

**修正后的实际方案：客户端事件驱动的预防性刷新**

既然无法检测，改为在**已知的高风险时机**主动触发刷新请求：

- 跨服跳转完成后（WaterdogPE 切服是隐身的最高频诱因）
- 维度切换后（下界 / 末地 / 自定义维度切换）
- 玩家自己切换到第三人称视角时

客户端在这些事件触发后，通过 PyRpc 主动请求 `RequestSkinRefreshEvent`，服务端只刷新请求者附近 128 格内的玩家（精准刷新，避免全服广播）。这比服务端定时全量广播更精准、开销更低，本质上是**预防性**刷新而非**反应性**修复。

### 2. 自适应刷新策略

当前定时任务是固定间隔。可增强为：

- ~~记录每次刷新后客户端的反馈（是否有玩家重新出现）~~ — 同样不可行，客户端无法感知渲染结果（原因同上）
- 在服务端侧统计高频时段（如跨服高峰期），动态提高刷新频率
